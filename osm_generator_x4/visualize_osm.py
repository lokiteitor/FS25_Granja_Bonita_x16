...
def main():
    ...
    # Set axes limits (Playable area is 0 to 4096 meters)
    ax.set_xlim(-100, 4296)
    ax.set_ylim(4296, -100) # Inverted Y-axis to match 0 at North/Top
    ...
    rect_playable = patches.Rectangle((0, 0), 4096, 4096,
                                      fill=False, edgecolor='#6366F1', linewidth=2.5, linestyle='--',
                                      label='Playable Area (4.1 km)')
    ...
    # Unique refs per way: a closed polygon repeats its first node to close the ring,
    # which is not a junction and must not paint an intersection dot.
    node_usage = {}
    for way in ways:
    ...
    # Set Title
    ax.set_title(f"OSM Layout Visualization - Playable 4.1km Area\nCenter: {lat_center:.6f}, {lon_center:.6f}",
