...
# Map parameters (x4 map: half the linear size of the x16 version, but field
# sizes are kept at their real-world values, so fewer fields fit).
lat_center = 43.145692357357156
lon_center = -95.1450786604236
size_m = 4096.0 # Playable area size (4.096 km)

# Local coordinate to Lat/Lon conversion
def local_to_global(x, y):
    # x: 0 to 4096 (West to East)
    # y: 0 to 4096 (North to South)
    delta_y = 2048.0 - y
    delta_x = x - 2048.0
...
def main():
    print("=== Generating OSM data for FS25 x4 map ===")
    ...
    # emitters below drop (FOREST_MIN_HA) - so the field boundaries must ignore it
    # too, or a field ends up with a bite taken out of it around a forest that is
    # never drawn. 1 px = 1 m2, so 1 ha = 10000 px.
    forest_mask = playable >= 37000.0
    lab_forest, n_forest = ndimage.label(forest_mask)
    if n_forest:
        sizes = np.atleast_1d(ndimage.sum(forest_mask, lab_forest, range(1, n_forest + 1)))
        keep = np.concatenate(([False], sizes >= 10000.0))
        dropped = int((~keep[1:]).sum())
        if dropped:
            print(f"   Ignoring {dropped} sub-hectare forest crumb(s) in the DEM mask.")
        forest_mask = keep[lab_forest]

    # Forest checking helpers
    def is_in_forest(x, y, buffer_m=5.0):
        ...
        x_max = min(4095, int(x + buffer_m))
        y_min = max(0, int(y - buffer_m))
        y_max = min(4095, int(y + buffer_m))

        sub = forest_mask[y_min:y_max+1, x_min:x_max+1]
        return sub.size > 0 and bool(sub.any())
    ...
    minlat, minlon = local_to_global(0, 4096) # South-West
    maxlat, maxlon = local_to_global(4096, 0) # North-East

    # 2. Yard 7 (Southern)
    # X: 12.5 to 1012.5, Y: 3833.5 to 4083.5 (1000m x 250m = 25 ha exactly).
    # The band south of the Southern Link Road is only ~250m tall, so the yard gets
    # its size by running east, swallowing what used to be pocket A.
    yard7_coords = [
        (12.5, 3833.5),
        (1012.5, 3833.5),
        (1012.5, 4083.5),
    ...
    # 2b. Field S_C1_1 (SW Hill)
    # X: 1027.5 to 1322.7, Y: 3833.5 to 4083.5 (~7.4 ha), 15m east of Yard 7.
    # This is the ground the south-western forest used to stand on: the DEM flattens
    # the old hill here (no 370m contour survives), and this field takes its place.
    # Named in the southern-pocket style, as pocket C between Yard 7 and pocket B.
    sw_hill_coords = [
        (1027.5, 3833.5),
        (1322.7, 3833.5),
        (1322.7, 4083.5),
        (1027.5, 4083.5),
        (1027.5, 3833.5) # Closed
    ]
    add_way(sw_hill_coords, {'landuse': 'farmland', 'name': 'Field S_C1_1 (7.4 ha)'})

    # 3. Yard Town (Town Farmyard)
    # X: 3879 to 4071, Y: 512 to 768
    yard_town_coords = [
        (3879.0, 512.0),
        (4071.0, 512.0),
        (4071.0, 768.0),
    ...
    # 5. East Canal (Canal)
    # X: 4059 to 4196, Y: 675.5 to 680.5
    canal_coords = [
        (4059.0, 675.5),
        (4196.0, 675.5),
        (4196.0, 680.5),
    ...
    # Passing 7.5m north of Town (Y: 512 - 7.5 = 504.5)
    # Spans from X: 0 to X: 4096.
    xs_sec = [3559.0, 3639.0, 3719.0, 3799.0, 3879.0]
    xs_ter_v = [800.0, 1600.0, 2400.0, 3200.0]
    xs_primary_all = sorted([3551.5] + xs_ter_v + xs_sec)
    primary_coords = [(0.0, 504.5)] + [(x, 504.5) for x in xs_primary_all] + [(4096.0, 504.5)]
    ...
    new_primary_coords_base.append((0.0, 3825.0))
    new_primary_coords_base.append((2534.0, 3825.0))

    # Curve 1
    c1_pts = bezier_curve((2534.0, 3825.0), (2684.0, 3825.0), (2790.065, 3718.935))
    new_primary_coords_base.extend(c1_pts[1:-1])

    # Segment 2: end of Curve 1 to start of Curve 2
    new_primary_coords_base.append((2790.065, 3718.935))
    new_primary_coords_base.append((3445.435, 3063.565))

    # Curve 2
    c2_pts = bezier_curve((3445.435, 3063.565), (3551.5, 2957.5), (3551.5, 2807.5))
    new_primary_coords_base.extend(c2_pts[1:-1])

    # Segment 3: end of Curve 2 to connection with northern primary road
    new_primary_coords_base.append((3551.5, 2807.5))
    new_primary_coords_base.append((3551.5, 768.0))
    new_primary_coords_base.append((3551.5, 640.0))
    new_primary_coords_base.append((3551.5, 512.0))
    new_primary_coords_base.append((3551.5, 504.5))

    # Set up interpolation variables for the road shape
    road_pts_filtered = [pt for pt in new_primary_coords_base if pt[0] < 3551.5]
    road_pts_filtered.sort(key=lambda pt: pt[0])
    road_xs_interp = [pt[0] for pt in road_pts_filtered] + [3551.5]
    road_ys_interp = [pt[1] for pt in road_pts_filtered] + [2807.5]
    ...
    forest_access_pt = (
        2790.065 + forest_access_t * (3445.435 - 2790.065),
        3718.935 + forest_access_t * (3063.565 - 3718.935)
    )
    ...
    # 8. Railway
    # Passing 7.5m north of Primary Road (Y: 504.5 - 7.5 = 497)
    # Parallel to Primary Road
    rail_coords = [(0.0, 497.0), (4096.0, 497.0)]
    add_way(rail_coords, {'railway': 'rail', 'name': 'Railway'})

    # 9. Secondary Roads (Grid in Town)
    ys_sec_v = [504.5, 512.0, 640.0, 768.0]
    for x in xs_sec:
        v_coords = [(x, y) for y in ys_sec_v]
        add_way(v_coords, {'highway': 'secondary'})

    xs_sec_h = [3551.5] + xs_sec
    ys_sec_h = [512.0, 640.0, 768.0]
    for y in ys_sec_h:
        h_coords = [(x, y) for x in xs_sec_h]
        add_way(h_coords, {'highway': 'secondary'})

    # The PLSS grid. Cells stay at the full 800m (~63 ha usable) size of the x16 map,
    # so the x4 map simply has fewer of them (5 columns x 4 rows). Defined here rather
    # than in section 9c, where the cells are filled, because section 9b below has to
    # know which of its roads a merged cell swallows.
    xs_grid_lines = [0.0] + xs_ter_v + [3551.5]
    ys_grid_lines = [504.5] + ys_ter_h + [3825.0]
    ...
    # 1. Select random cells to place forests. The x16 map holds 10 of them in 81
    # cells; this map has 20 cells, so 3 keeps the same density.
    NUM_RANDOM_FORESTS = 3
    import random
    candidates = []
    clear_cells = set()
    for i in range(len(xs_grid_lines) - 1):
        ...
        for j in range(len(ys_grid_lines) - 1):
            ...
            # reshuffle every draw and move all the forests, so the check is applied to the
    ...
    selected_cells = set([c for c in candidates if c in clear_cells][:NUM_RANDOM_FORESTS])

    # PLSS cells (x-index, y-index into xs_grid_lines / ys_grid_lines) kept as forest
    # or yard instead of farmland. The x16 map converts one narrow strip against the
    # Southern Link Road each way; on this map every cell is a full-size (~60 ha)
    # parcel, far too large to give away, so both sets stay empty.
    wooded_cells = set()
    yard_cells = set()
    ...
    for x in xs_ter_v:
        ...
        if not is_in_forest(x, y_int, buffer_m=5.0):
            ...
            y_forest_limit = get_forest_limit_y(x, 504.5, y_int, buffer_m=5.0)
    ...
    print(f"   Generating PLSS farmlands (fields) and {NUM_RANDOM_FORESTS} random forests...")
    for i in range(len(xs_grid_lines) - 1):
        x_a = xs_grid_lines[i]
        x_b = xs_grid_lines[i+1]
        for j in range(len(ys_grid_lines) - 1):
            ...
            if (i, j) in selected_cells:
                ...
                # 1. Right Field
                xs_sample = np.linspace(x_f_end + 10.0, x_b - 5.0, 33)
                poly_top = []
                poly_bottom = []
                valid = True
                for x in xs_sample:
                ...
                # 2. Bottom Field
                xs_sample = np.linspace(x_a + 5.0, x_b - 5.0, 33)
                poly_top = []
                poly_bottom = []
                valid = True
                for x in xs_sample:
    ...
    # ...except at the eastern end of every strip, which faces the town (x 3559-4071)
    ...
    # backwards from x=4081, so the block finishes flush with the edge and no column ends
    # up as a runt.
    NORTH_FIXED_BANDS = [(5.0, 4), (10.0, 2)]  # (hectares, number of columns)

    # Strip 2 uses its own fixed block: the second column from the eastern edge is a
    # 10 ha one that the town-facing yard claims (see north_yards below), so the yard
    # comes out as an exact 10 ha horizontal rectangle instead of a merged run of
    # 5 ha smallholdings.
    NORTH2_FIXED_BANDS = [(5.0, 1), (10.0, 1), (5.0, 2), (10.0, 2)]

    def pack_strip_horiz(y_start, y_end, seed, bands_spec=NORTH_FIXED_BANDS,
                         west_cols=None):
        ...
        # Exact-size columns reserved at the western edge (the western yard and its
        # smallholdings), listed west to east in hectares. Fixed, so neither the field
        # merge nor the runt-absorbing pass can change their size; the random mix
        # packs into whatever is left between them and the eastern block.
        if west_cols:
            for col_ha in west_cols:
                w = (col_ha * 10000.0) / field_h
                fields.append((x_curr, x_curr + w, True))
                x_curr += w + 10.0

        # Place the fixed bands first, walking west from the eastern edge.
        bands = []
        x_edge = 4081.0
        for band_ha, count in bands_spec:
    ...
    # every column of that strip the span touches joins the yard. Spans rather than
    # column indices, because the indices shift whenever the packing changes. Both
    # yards sit on exact 10 ha fixed columns (367.6m x 272m, horizontal rectangles):
    # (2, 15, 20) is the reserved western-corner column, and (2, 3600, 3800) is the
    # 10 ha column of NORTH2_FIXED_BANDS facing the town across the railway.
    north_yards = {(2, 15.0, 20.0), (2, 3600.0, 3800.0)}

    # The band between the northern edge and the railway is half as tall as on the x16
    # map, so instead of five 180m strips there are two: one at the original 180m and a
    # deeper 272m one, which keeps the 5/10/20 ha parcels at sensible aspect ratios.
    strips = [
        (15.0, 195.0),
        (205.0, 477.0)
    ]
    ...
    # Boundary North
    add_way([(15.0, 15.0), (4081.0, 15.0)], {'highway': 'tertiary'})
    # Gaps horizontal roads
    add_way([(0.0, 200.0), (4096.0, 200.0)], {'highway': 'tertiary'})

    # Boundary West (15m offset)
    add_way([(15.0, 15.0), (15.0, 504.5)], {'highway': 'tertiary'})
    # Boundary East (15m offset, which is 4081.0)
    add_way([(4081.0, 15.0), (4081.0, 504.5)], {'highway': 'tertiary'})

    # Generate fields & vertical roads for each strip
    for s_idx, (y_s, y_e) in enumerate(strips):
        field_h = y_e - y_s
        if s_idx == 1:
            # Strip 2 carries the two 10 ha yards: the first western column and the
            # 10 ha column of its own eastern block (see NORTH2_FIXED_BANDS and
            # north_yards above). Behind the western yard come two 5 ha and two
            # 10 ha fixed columns, mirroring the eastern block, and the random mix
            # shrinks to whatever is left in the middle.
            raw = pack_strip_horiz(y_s, y_e, seed=(303 + s_idx),
                                   bands_spec=NORTH2_FIXED_BANDS,
                                   west_cols=[10.0, 5.0, 5.0, 10.0, 10.0])
        else:
            raw = pack_strip_horiz(y_s, y_e, seed=(303 + s_idx))
        parcels = merge_strip_fields(raw, field_h, s_idx)
        ...
        for rx in roads:
            if s_idx == 0:
                ...
                # Gap 1 to primary road (crosses railway at 497)
                add_way([(rx, 200.0), (rx, 497.0), (rx, 504.5)], {'highway': 'tertiary'})
    ...
    # The band between the town and the eastern edge is only ~522m wide on this map,
    # so instead of three 346m columns there is a single full-width one; the 30/45 ha
    # fields then come out roughly square.
    col_w = 522.0
    col_xs = [
        (3559.0, 4081.0)
    ]
    ...
    for c_idx, (x_s, x_e) in enumerate(col_xs):
        ...
        x_road_start = 3551.5 if c_idx == 0 else vertical_gaps[c_idx - 1]
        x_road_end = 4081.0 if c_idx == len(col_xs) - 1 else vertical_gaps[c_idx]

        while True:
            # Get forest limit Y for this column width
            col_y_limit = min(
                min(get_road_y(x) - 15.0, get_forest_limit_y(x, y_curr, 4096.0, buffer_m=15.0))
    ...
    for g_idx, rx in enumerate(vertical_gaps):
        ...
        add_way([(rx, 768.0), (rx, y_lim)], {'highway': 'tertiary'})

    # Boundary East road extension down to the last road in the column
    y_lim_east = col_last_ys[-1]
    add_way([(4081.0, 768.0), (4081.0, y_lim_east)], {'highway': 'tertiary'})
    ...
    # column is a fixed 10 ha, so there is no seed to pass in. The band south of the
    # Southern Link Road is half as tall as on the x16 map, so it holds one strip of
    # 10 ha columns instead of two.
    def pack_south_pocket(x_start, x_end, p_name):
        y_s1, y_e1 = 3840.0, 4066.0

        field_h = 226.0
        w_10ha = 442.5

        # Add horizontal road at Y = 4071.0 (southern boundary of the strip)
        add_way([(x_start, 4071.0), (x_end, 4071.0)], {'highway': 'tertiary'})

        # Add boundary vertical roads
        add_way([(x_start, 3825.0), (x_start, 4071.0)], {'highway': 'tertiary'})
        add_way([(x_end, 3825.0), (x_end, 4071.0)], {'highway': 'tertiary'})
        ...
        for strip_idx, (y_s, y_e) in enumerate([(y_s1, y_e1)]):
        ...
        # Vertical roads in the gaps between columns.
        for x_s, x_e in cols[:-1]:
            add_way([(x_e + 5.0, 3825.0), (x_e + 5.0, 4071.0)], {'highway': 'tertiary'})

    # Pocket B: between the SW Hill field (pocket C) and Southern Link Road curve 1.
    # Pocket A and the south-western forest are gone: Yard 7 and Field S_C1_1 cover
    # the ground they used to sit on.
    pack_south_pocket(1337.7, 2519.0, p_name="B")

    # 10. Forest polygons (elevations >= 370m)
    def get_border(pt):
        x, y = pt
        if math.isclose(x, 0.0, abs_tol=1e-3): return 'W'
        if math.isclose(x, 4096.0, abs_tol=1e-3): return 'E'
        if math.isclose(y, 0.0, abs_tol=1e-3): return 'N'
        if math.isclose(y, 4096.0, abs_tol=1e-3): return 'S'
        return None
    ...
    def get_forest_polygons():
        grid_size = 513
        idx = np.linspace(0, 4095, grid_size, dtype=int)
        playable_sub = playable[idx, :][:, idx]

        x_grid = np.linspace(0, 4096, grid_size)
        y_grid = np.linspace(0, 4096, grid_size)
    ...
    def poly_area_ha(poly):
        s = 0.0
        for k in range(len(poly) - 1):
            s += poly[k][0] * poly[k+1][1] - poly[k+1][0] * poly[k][1]
        return abs(s) / 2.0 / 10000.0

    # Specks below this size are dropped: flattening the old south-western hill under
    # Field S_C1_1 leaves the odd 370m crumb poking through between the 5-sample forest
    # checks of the PLSS fields, and a sub-hectare clump of wood in the middle of a
    # field is noise, not a forest.
    FOREST_MIN_HA = 1.0
    ...
    for i, poly in enumerate(forest_polys):
        if poly_area_ha(list(poly) + [poly[0]] if poly[0] != poly[-1] else list(poly)) < FOREST_MIN_HA:
            print(f"   Skipped forest speck {i+1} ({len(poly)} nodes, < {FOREST_MIN_HA:.0f} ha).")
            continue
        # No clipping against the Southern Link Road here: on the x16 map the
        # south-western forest needed cutting back into the gap between the two
        # southern pockets, but on this map Field S_C1_1 sits on that hill, flattened
        # by the DEM, so no 370m contour survives south of the road. The 370m
        # contour is followed as-is; the roads and fields already keep clear of it
    ...
    def build_infill_polygons():
        n = int(round(4096.0 / INFILL_SCALE_M))
        ...
        def trace(selected_ids):
            ...
            for seg in cs.allsegs[0]:
                pts = [(min(max(px, 0.0), 4096.0), min(max(py, 0.0), 4096.0)) for px, py in seg]
                if pts[0] != pts[-1]:
                    pts.append(pts[0])
                ...
                if pts[0] != pts[-1]:
                    pts.append(pts[0])
                # Sub-hectare slivers are raster artifacts, not pockets: around a road
                # junction the corridor can leave a crumb of "open" pixels that the
                # dilation stitches across the road, and its simplified outline then
                # cuts straight over the carriageway. Same threshold as the forest
                # contour specks.
                if poly_area_ha(pts) < FOREST_MIN_HA:
                    print(f"   Skipped infill sliver ({len(pts)} nodes, "
                          f"{poly_area_ha(pts):.1f} ha).")
                    continue
                polygons.append(pts)
    ...
    FOREST_SUMMIT = (4070.0, 3970.0)
    FOREST_SEED = (3750.0, 3500.0)  # a point known to sit inside the south-eastern forest

    def build_forest_tracks():
        s = 4.0
        gn = int(round(4096.0 / s))
        ...
        for _ in range(400):
            ...
            if not (40.0 <= x <= 4056.0 and 40.0 <= y <= 4056.0):
                break
            haul.append((x, y))
