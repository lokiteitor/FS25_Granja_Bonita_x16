...
def get_road_x_global(y_m, offset_m=2048.0, S_playable=4096.0):
...
def main():
    t_start = time.time()
    print("=== FS25 8K New DEM Generator (Exactly 8192x8192 with 4096 Playable + 4096 Offset) ===")

    # Configuration (x4 map: 4096 playable + 2048 offset per side = 8192x8192 canvas)
    S_px = 8192   # Heightmap resolution in pixels (exactly 8192x8192)
    S_m = 8192    # Heightmap size in meters (8192x8192m)
    scale_m_to_px = 1.0  # 1 pixel = 1 meter
    offset_m = 2048.0   # Playable area (4096x4096) centered in the 8192 canvas (2048 offset per side)

    seed = 20260608
    np.random.seed(seed)

    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dem_path = os.path.join(script_dir, "dem_new_8k.png")
    output_vis_path = os.path.join(script_dir, "dem_new_visual_8k.png")
    output_detail_vis_path = os.path.join(script_dir, "dem_new_visual_detail_8k.png")
    ...
    # Compute background mountain weight (0 inside playable area x,y in [2048, 6144] meters, rises to 1.0 at 512m away)
    dx_bg = np.maximum(0.0, np.maximum(2048.0 - x_m, x_m - 6144.0))
    dy_bg = np.maximum(0.0, np.maximum(2048.0 - y_m, y_m - 6144.0))
    ...
    # Center of the dome: SE corner of playable area (6144.0, 6144.0)
    # Radius: 1250 meters
    dx_mountain = x_m - 6144.0
    dy_mountain = y_m - 6144.0
    ...
    # x_m in [2048, 6144] and y_m in [2048, 2816] (y_osm < 768 + offset)
    rx0_m, rx1_m = 2048.0, 6144.0
    ry0_m, ry1_m = 2048.0, 2816.0
    ...
    vis_scale = 8  # Upscaled to match 1024x1024 visual dimension (8192 / 8 = 1024)
    ...
    # Define scale from meters to visualization coordinates
    scale_m_to_vis = scale_m_to_px / vis_scale

    # --- Map 1: Full 8K Map View ---
    print("   Generating full map visualization...")
    fig, ax = plt.subplots(figsize=(12, 12), dpi=150)
    ax.imshow(hs, extent=[0, 8192, 8192, 0])

    # Grid and tick labels for 8.2km canvas
    ...
    rect_playable = plt.Rectangle((2048.0, 2048.0), 4096.0, 4096.0,
    ...
    x_range_all = np.arange(1024) * 8.0
    ...
    p_start = int(2048.0 / vis_scale)  # 256
    p_end = int(6144.0 / vis_scale)    # 768
    ...
    # Draw natural mountain shape contour lines for Bosque at 340m, 370m, 400m, 430m elevation
    x_range = np.arange(p_end - p_start) * 8.0
